...
use revm::db::{CacheDB, AccountState};
use revm::primitives::{
    hash_map::Entry, AccountInfo, Address, Bytecode, Bytes, B256,
    U256,
};
...
impl<R: ExecutionRpc> StateProvider for ExecutionClient<R> {
    ...
    fn fetch_account(&mut self, address: Address) -> Result<AccountInfo, Self::Error> {
        /* TODO: slots */
        match futures::executor::block_on(self.get_account(&EthersAddress::from_slice(address.as_slice()), None, Latest)) {
            Ok(acc) => {
                Ok(AccountInfo::new(acc.balance.into(), acc.nonce, acc.code_hash.to_fixed_bytes().into(), Bytecode::new_raw(Bytes::from_iter(acc.code.into_iter()))))
            }
            Err(_) => {
                Err(())
            }
    ...
    fn fetch_storage(&mut self, address: Address, index: U256) -> Result<U256, Self::Error> {
        ...
        match futures::executor::block_on(self.get_account(&EthersAddress::from_slice(address.as_slice()), Some(slots.as_ref()), Latest)) {
            Ok(acc) => {
    ...
    fn fetch_block_hash(&mut self, number: U256) -> Result<B256, Self::Error> {
        match futures::executor::block_on(self.get_block(Number(number.as_limbs()[0]), false)) {
            Ok(block) => {
                Ok(B256::from_slice(block.hash.to_fixed_bytes().as_slice()))
            }
            Err(_) => {
                Err(())
            }
